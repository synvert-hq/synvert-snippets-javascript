...
if (this.options.defaultImport) {
  ...
  unlessExistNode(
    ...
    () => {
      withNode(
        { nodeType: "ImportDeclaration", importClause: { name: undefined }, moduleSpecifier: { text: moduleSpecifier } },
        () => {
...
if (this.options.namedImport) {
  ...
  unlessExistNode(
    ...
    () => {
      unlessExistNode(
        { nodeType: "ImportDeclaration", importClause: { namedBindings: { elements: { includes: namedImport } } }, moduleSpecifier: { text: moduleSpecifier } },
        () => {
          ifAllNodes(
            { nodeType: "ImportDeclaration" }, { match: { moduleSpecifier: { text: { not: moduleSpecifier } } } },
            () => {
              insertBefore(`import { ${namedImport} } from "${moduleSpecifier}";`);
            },
            () => {
              // import React from "react"
              withNode({ nodeType: "ImportDeclaration", importClause: { name: { not: undefined }, namedBindings: undefined }, moduleSpecifier: { text: moduleSpecifier } }, () => {
                insert(`, { ${namedImport} }`, { to: "importClause.name", at: "end" });
              });
              // import React, { Component } from "react"
              withNode({ nodeType: "ImportDeclaration", importClause: { name: { not: undefined }, namedBindings: { not: undefined } }, moduleSpecifier: { text: moduleSpecifier } }, () => {
                insert(`, { ${namedImport} }`, { to: "importClause.name", at: "end" });
              });
              // import { Component } from "react"
              withNode({ nodeType: "ImportDeclaration", importClause: { name: undefined, namedBindings: { not: undefined } }, moduleSpecifier: { text: moduleSpecifier } }, () => {
...
if (this.options.namespaceImport) {
  ...
  unlessExistNode(
    { nodeType: "ImportDeclaration", importClause: { namedBindings: { nodeType: "NamespaceImport", name: namespaceImport } }, moduleSpecifier: { text: moduleSpecifier } },
