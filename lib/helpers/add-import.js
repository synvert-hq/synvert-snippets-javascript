...
new Synvert.Helper("helpers/add-import", function (options) {
  ...
  if (options.namedImport) {
    ...
    for (const name of namedImports) {
      withNode(
        { nodeType: "ImportDeclaration", importClause: { namedBindings: { elements: { includes: name } } } },
        () => {
    ...
    if (unusedNamedImports.length > 0) {
      unlessExistNode(
        ...
        () => {
          insertBefore(
            appendSemicolon(`import { ${unusedNamedImports.join(", ")} } from ${wrapWithQuotes(moduleSpecifier)}`),
          );
        },
        () => {
          ifAllNodes(
            ...
            () => {
              ...
              withNode(
                ...
                () => {
                  insert(`, ${unusedNamedImports.join(", ")}`, {
                    to: "importClause.namedBindings.elements.-1",
                    at: "end",
                  });
                },
              );
