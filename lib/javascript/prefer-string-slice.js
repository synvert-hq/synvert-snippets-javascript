...
new Synvert.Rewriter("javascript", "prefer-string-slice", () => {
  ...
  withinFiles(Synvert.ALL_JS_FILES, () => {
    findNode(
      `.CallExpression[expression=.PropertyAccessExpression[name=substr]][arguments.length=2]`,
      () => {
    ...
    findNode(
      `.CallExpression[expression=.PropertyAccessExpression[name=substring]][arguments.length=2]`,
      () => {
        replace("expression.name", { with: "slice" });
      }
    );
