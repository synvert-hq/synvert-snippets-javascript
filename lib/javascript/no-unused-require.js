      ...
      ifAllNodes(
        ...
        () => {
          findNode(
            ...
            () => {
              remove({ andComma: true }); // remove default require declarator
            }
          );
          findNode(
            ...
            () => {
              ifAllNodes(
                ...
                () => {
                  remove({ andComma: true }); // remove all require declarators in { }
                },
                () => {
                  findNode({ nodeType: "BindingElement", name: { in: unusedNames } }, () => {
                    remove({ andComma: true }); // remove single require declarator in { }
                  });
                }
              );
            }
