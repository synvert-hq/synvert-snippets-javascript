  ...
  withinFiles(Synvert.ALL_FILES, function () {
    ...
    findNode(".VariableDeclarationList", () => {
      ifAllNodes(
        ...
        () => {
          findNode({ nodeType: "VariableDeclaration", name: { nodeType: "Identifier", escapedText: { in: unusedNames } }, initializer: { nodeType: "CallExpression", expression: "require" } },
            () => {
              remove(); // remove default require declarator
            }
          );
          findNode(".VariableDeclaration[name=.ObjectBindingPattern][initializer=.CallExpression[expression=require]]", () => {
            ifAllNodes(
