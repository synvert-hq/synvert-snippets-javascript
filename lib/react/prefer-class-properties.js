...
const { IfOnlyExistCondition } = require("../../../synvert-core-javascript/lib/condition");
...
new Synvert.Rewriter("react", "preferClassProperties", () => {
  ...
  withFiles("**/*.{js,jsx}", function () {
    withNode({ type: "MethodDefinition", kind: "constructor" }, () => {
      ifOnlyExistNode({ type: "ExpressionStatement", expression: { type: "CallExpression", callee: { type: 'Super' }, arguments: { length: this.currentNode.value.params.length } } }, () => {
        remove();
      });
    });
  });
