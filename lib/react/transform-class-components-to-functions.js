...
new Synvert.Rewriter("react", "transform-class-components-to-functions", () => {
  ...
  withinFiles(Synvert.ALL_FILES, function () {
    ...
    findNode(".ClassDeclaration[superClass=Component]", () => {
      ...
      findNode(
        ".VariableDeclaration[declarations.length=1][declarations.0.init=.MemberExpression[object=this][property=props]]",
        () => {
...
${functionBody.join("\n")}
  ...
  withinFiles(`{${Object.keys(convertedFiles).join(",")},}`, function () {
    ...
    findNode(
      `.VariableDeclarator[id=${componentName}] .ArrowFunctionExpression[params.length=1][params.0=props]`,
      () => {
    ...
    findNode(
      `.VariableDeclaration[declarations.length=1][declarations.0.init=.MemberExpression[object=this][property=state]]`,
      () => {
        remove();
      }
    );
    ...
    // const render = () => {}
    findNode(
      ".VariableDeclaration[declarations.length=1][declarations.0=.VariableDeclarator[id=render][init=.ArrowFunctionExpression]]",
      () => {
    ...
    findNode(
      `:has(.CallExpression[callee=useState])
                .ImportDeclaration[source.value=react][specifiers.0=.ImportDefaultSpecifier[local=React]]
