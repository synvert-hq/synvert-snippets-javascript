  ...
  withinFiles(Synvert.ALL_FILES, function () {
    ...
    findNode(".ClassDeclaration[heritageClauses.0.types.0=Component]", () => {
      ...
      findNode(".MethodDeclaration[name=render]", () => {
        functionBody.push(
          indent(this.mutationAdapter.getSource(this.currentNode, { fixIndent: true }), Synvert.Configuration.tabWidth).replace(
            /render ?\(\)/,
            "const render = () =>"
          )
        );
...
${functionBody.join("\n")}
  ...
  withinFiles(`{${Object.keys(convertedFiles).join(",")},}`, function () {
    ...
    findNode(
      ...
      () => {
        const column = this.mutationAdapter.getIndent(this.currentNode);
        const body = [];
        ...
        replaceWith(indent(body.join("\n"), column, { skipFirstLine: true }))
      }
    );
