...
new Synvert.Rewriter("react", "transform-class-components-to-functions", () => {
  ...
  withinFiles(Synvert.ALL_FILES, function () {
    ...
    findNode(".ClassDeclaration[superClass=Component]", () => {
      ...
      findNode(".VariableDeclaration[declarations.length=1][declarations.first.init=.MemberExpression[object=this][property=props]]", () => {
...
${functionBody.join("\n")}
  ...
  withinFiles(`{${Object.keys(convertedFiles).join(",")},}`, function () {
    ...
    findNode(`.VariableDeclarator[id=${componentName}] .ArrowFunctionExpression[params.length=1][params.first=props]`, () => {
    ...
    findNode(`.VariableDeclaration[declarations.length=1][declarations.first.init=.MemberExpression[object=this][property=state]]`, () => {
      remove();
    });

    // this.props.name => name
    findNode(`.MemberExpression[object=.MemberExpression[object=this][property=props]][property NOT IN (${conflictNames.join(' ')})]`, () => {
      deleteNode(["object", "dot"]);
    });

    // this.props.name => props.name
    findNode(`.MemberExpression[object=.MemberExpression[object=this][property=props]][property IN (${conflictNames.join(' ')})]`, () => {
    ...
    // const render = () => {}
    findNode(".VariableDeclaration[declarations.length=1][declarations.first=.VariableDeclarator[id=render][init=.ArrowFunctionExpression]]", () => {
    ...
    findNode(`:has(.CallExpression[callee=useState])
    ...
    if (useEffect) {
      ...
      findNode(`.ImportDeclaration[source.value=react][specifiers.first=.ImportDefaultSpecifier[local=React]]
