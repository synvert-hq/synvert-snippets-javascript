...
new Synvert.Rewriter("react", "transform-class-components-to-functions", () => {
  ...
  withinFiles(Synvert.ALL_FILES, function () {
    const currentFilePath = this.currentNode.getSourceFile().fileName;
    findNode(".ClassDeclaration[heritageClauses.0.types.0=Component]", () => {
      let functionBodyIndent = mutationAdapter().getIndent(this.currentNode);
      const functionBody = [];
      ...
      findNode(
        ...
        () => {
          this.currentNode.declarations[0].name.elements.forEach((element) =>
            addThisProp(currentFilePath, mutationAdapter().getSource(element))
          );
        }
      );
      ...
      findNode(".PropertyAccessExpression[expression=.PropertyAccessExpression[expression=this][name=props]]", () => {
        addThisProp(currentFilePath, mutationAdapter().getSource(this.currentNode.name));
      });
      ...
      findNode(".PropertyDeclaration[name=state]", () => {
        this.currentNode.initializer.properties.forEach((property) => {
          ...
          const value = mutationAdapter().getSource(property.initializer);
          functionBody.push(
            indent(
      ...
      findNode(".PropertyDeclaration[initializer=.ArrowFunction]", () => {
        ...
        const value = mutationAdapter().getSource(this.currentNode.initializer, { fixIndent: true });
      ...
      if (lifecycleMethods.length > 0) {
        ...
        lifecycleMethods.forEach((lifecycleMethod) => {
          const lines = mutationAdapter().getSource(lifecycleMethod, { fixIndent: true }).split("\n");
          functionBody.push(
      ...
      findNode(".MethodDeclaration[name=render]", () => {
        functionBody.push(
          indent(mutationAdapter().getSource(this.currentNode, { fixIndent: true }), DEFAULT_INDENT).replace(
            /render ?\(\)/,
            "const render = () =>"
          )
        );
...
${functionBody.join("\n")}
  ...
  withinFiles(`{${Object.keys(convertedFiles).join(",")},}`, function () {
    const currentFilePath = this.currentNode.getSourceFile().fileName;
    const { componentName, useEffect, thisMethods, thisProps } = convertedFiles[currentFilePath];
    ...
    findNode(
      ...
      () => {
        deleteNode(["expression.expression", "expression.dot"]);
      }
    );
    ...
    findNode(".CallExpression[expression=.PropertyAccessExpression[expression=this][name=setState]]", () => {
      ...
      findNode(".ObjectLiteralExpression .PropertyAssignment", () => {
        properties.push({
          ...
          value: mutationAdapter().getSource(this.currentNode.initializer),
        });
      });
    ...
    findNode(
      ...
      () => {
        ...
        this.currentNode.declarations[0].initializer.body.statements.forEach((statement) => {
          body.push(mutationAdapter().getSource(statement, { fixIndent: true }));
        });
