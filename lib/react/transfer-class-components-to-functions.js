  ...
  withinFiles(Synvert.ALL_FILES, function () {
    withNode({ type: "ClassDeclaration", superClass: { object: 'React', property: 'Component' } }, () => {
      ...
      withNode({ type: "PropertyDefinition", key: "state" }, () => {
        this.currentNode.value.properties.forEach(property => states.push({
          name: property.key.name,
          value: property.value.value
        }));
      });
      const functionBody = [];
      states.forEach(({ name, value }) => functionBody.push(`const [${name}, set${name.replace(/^\w/, (c) => c.toUpperCase())}] = useState(${value});`))
