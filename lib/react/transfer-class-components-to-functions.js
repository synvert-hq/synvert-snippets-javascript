  ...
  withinFiles(Synvert.ALL_FILES, function () {
    ...
    withNode({ type: "ClassDeclaration", superClass: 'Component' }, () => {
      ...
      withNode({ type: "VariableDeclaration", declarations: { length: 1, first: { init: { type: "MemberExpression", object: "this", property: "props" } } } }, () => {
        this.currentNode.declarations[0].id.properties.forEach(property => addThisProp(currentFilePath, property.toSource()));
      });
      ...
      if (lifecycleMethods.length > 0) {
        ...
        lifecycleMethods.forEach(lifecycleMethod => {
          ...
          functionBody.push(indent(lines.map(line => line.length > 0 ? '// ' + line : '//').join("\n"), DEFAULT_INDENT));
        });
      }
...
${functionBody.join("\n")}
  ...
  withinFiles(`{${Object.keys(convertedFiles).join(',')},}`, function () {
    ...
    withNode({ type: "VariableDeclarator", id: componentName }, () => {
      withNode({ type: "ArrowFunctionExpression", params: { length: 1, first: "props" } }, () => {
        if (thisProps.length === 0) {
          ...
          replace("params.0", { with: `{ ${[...new Set(thisProps.filter(prop => !thisMethods.includes(prop)))].join(", ")}, ...props }` });
        }
      });
    });
    ...
    // this.props.name => name
    withNode({ type: "MemberExpression", object: { type: "MemberExpression", object: 'this', property: 'props' }, property: { notIn: conflictNames } }, () => {
      deleteNode(['object', 'dot']);
    });

    // this.props.name => props.name
    withNode({ type: "MemberExpression", object: { type: "MemberExpression", object: 'this', property: 'props' }, property: { in: conflictNames } }, () => {
