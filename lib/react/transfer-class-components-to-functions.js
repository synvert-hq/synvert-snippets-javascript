  ...
  withinFiles(Synvert.ALL_FILES, function () {
    withNode({ type: "ClassDeclaration", superClass: "Component" }, () => {
      ...
      withNode(
        ...
        () => {
          this.currentNode.declarations[0].id.properties.forEach((property) =>
            addThisProp(this.currentFilePath, property.toSource())
          );
        }
      );
      ...
      withNode({ type: "MethodDefinition", key: { in: lifecycleMethods } }, () => {
        if (!todoAdded) {
          functionBody.push("  // Synvert TODO: convert lifecycle methods to useEffect");
          todoAdded = true;
        }
      ...
      withNode({ type: "MethodDefinition", key: "render" }, () => {
        functionBody.push(
          indent(this.currentNode.fixIndentToSource(), DEFAULT_INDENT).replace("render()", "const render = () =>")
        );
      });
