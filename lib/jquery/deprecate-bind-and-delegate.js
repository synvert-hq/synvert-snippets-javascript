...
new Synvert.Rewriter("jquery", "deprecate-bind-and-delegate", () => {
  ...
  withinFiles(Synvert.ALL_JS_FILES, function () {
    ...
    findNode(".CallExpression[callee=.MemberExpression[object IN (/^\\$/ /^jQuery/)][property=bind]][arguments.length>=2]", () => {
    ...
    // =>
    // $(this).off("click");
    findNode(".CallExpression[callee=.MemberExpression[object IN (/^\\$/ /^jQuery/)][property=unbind]][arguments.length>=1]", () => {
    ...
    findNode(".CallExpression[callee=.MemberExpression[object IN (/^\\$/ /^jQuery/)][property=delegate]][arguments.length>=3]", () => {
      replace("callee.property", { with: "on" });
    ...
    // =>
    // $(this).off();
    findNode(".CallExpression[callee=.MemberExpression[object IN (/^\\$/ /^jQuery/)][property=undelegate]][arguments.length<2]", () => {
      replace("callee.property", { with: "off" });
    });
