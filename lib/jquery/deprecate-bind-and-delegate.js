...
new Synvert.Rewriter("jquery", "deprecate-bind-and-delegate", () => {
  ...
  withinFiles(Synvert.ALL_FILES, function () {
    withNode({ type: "CallExpression", callee: { type: "MemberExpression", object: { in: [/^\$/, /^jQuery/] }, property: 'bind' }, arguments: { length: { gte: 2 } } }, () => {
      replace('callee.property', { with: 'on' });
    });

    withNode({ type: "CallExpression", callee: { type: "MemberExpression", object: { in: [/^\$/, /^jQuery/] }, property: 'unbind' }, arguments: { length: { gte: 1 } } }, () => {
      replace('callee.property', { with: 'off' });
    });

    withNode({ type: "CallExpression", callee: { type: "MemberExpression", object: { in: [/^\$/, /^jQuery/] }, property: 'delegate' }, arguments: { length: { gte: 3 } } }, () => {
      replace('callee.property', { with: 'on' });
      replace('arguments.0', { with: '{{arguments.1}}' });
      replace('arguments.1', { with: '{{arguments.0}}' });
    });

    withNode({ type: "CallExpression", callee: { type: "MemberExpression", object: { in: [/^\$/, /^jQuery/] }, property: 'undelegate' }, arguments: { length: { lt: 2 } } }, () => {
      replace('callee.property', { with: 'off' });
    });

    withNode({ type: "CallExpression", callee: { type: "MemberExpression", object: { in: [/^\$/, /^jQuery/] }, property: 'undelegate' }, arguments: { length: { gte: 2 } } }, () => {
      replace('callee.property', { with: 'off' });
      replace('arguments.0', { with: '{{arguments.1}}' });
      replace('arguments.1', { with: '{{arguments.0}}' });
    });
