...
new Synvert.Rewriter("jquery", "deprecate-ajax-success-error-and-complete", () => {
  ...
  withinFiles(Synvert.ALL_FILES, function () {
    withNode({ type: "CallExpression", callee: { type: "MemberExpression", object: '$', property: 'ajax' }, arguments: { length: 1, first: { type: 'ObjectExpression' } } }, () => {
      ...
      if (successFunctionNode) {
        insert("\n" + indent(`.done(${successFunctionNode.value.fixIndentToSource()})`, column), { at: 'end' } );
      }
      if (errorFunctionNode) {
        insert("\n" + indent(`.fail(${errorFunctionNode.value.fixIndentToSource()})`, column), { at: 'end' } );
      }
      if (completeFunctionNode) {
        insert("\n" + indent(`.always(${completeFunctionNode.value.fixIndentToSource()})`, column), { at: 'end' } );
      }
    });
  });
});
