...
new Synvert.Rewriter("jquery", "use-camelcased-data-name", () => {
  ...
  withinFiles(Synvert.ALL_FILES, function () {
    findNode(
      `.CallExpression
        [expression=.PropertyAccessExpression[expression IN (/^\\$/ /^jQuery/)][name=data]]
        [arguments.length IN (1 2)][arguments.0=.StringLiteral]`,
      () => {
        const dataKey = this.currentNode.arguments[0].text;
        const quote = this.mutationAdapter.getSource(this.currentNode.arguments[0])[0];
        if (dataKey.includes("-")) {
